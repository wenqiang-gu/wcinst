...
wcinst-sbnd-init(){
  ...
  if [ "$larinfo" = "n" ];then
    ...
    echo -n "sbndcode_version="
    read sbndcode_version
  fi
...
sbndcode_version=${sbndcode_version}
EOF
...
wcinst-sbnd-bootstrap(){
 ...
 # wcdo-mrb-add-source  sbndcode $sbndcode_version  $sbndcode_version
 ...
 ./wcb -p --notests --install-config=sbnd install
 setup wirecell wctdev -q e19:prof
...
setup sbndcode ${sbndcode_version} -q e19:prof
export UPS_OVERRIDE=''
...
wcinst-sbnd-help(){
  ...
  usage:
    ...
    - get wirecell toolkit in three steps
      - ./wcinst-sbnd.sh init # in host shell
      - ./wcinst-sbnd.sh bootstrap # in singularity container
